...
class _RiskAssessmentScreenState extends State<RiskAssessmentScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Investment Risk Assessment')),
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButton<String>(
              ...
              items: RiskAssessmentService.goalScores.keys
                  .map((g) => DropdownMenuItem(value: g, child: Text(g)))
                  .toList(),
            ),
            const SizedBox(height: 20),

            const Text('Time Horizon (years):'),
            Slider(
              value: _timeHorizon.toDouble(),
              min: 1,
              max: 30,
              divisions: 29,
              label: _timeHorizon.toString(),
              onChanged: (val) => setState(() => _timeHorizon = val.toInt()),
            ),
            ...
            ElevatedButton(
              onPressed: () {
                final score = RiskAssessmentService.computeScore(
                  ...
                  timeHorizon: _timeHorizon.toDouble(),
                ...
                final profile = RiskAssessmentService.profileFromScore(score);
                showDialog(
                  context: context,
              ...
              child: const Text('Calculate Risk Profile'),
            )
          ],
        ),
      ),
...
class RiskAssessmentService {
  ...
  Future<RiskAssessment?> getLatestForUser(String? userId) async {
    ...
    return row == null ? null : RiskAssessment.fromData(row, db);
  }
}
